...
async def _migration_013_quest_templates(conn) -> None:
    """Create the ``quest_templates`` table (quest templates MVP).

    Per-user reusable quest text bank — see ``db.schema.quest_templates``.
    Standard ``create_all(checkfirst=True)`` pattern; the table is new so
    no ALTERs needed.
    """
    await conn.run_sync(lambda sync_conn: metadata.create_all(sync_conn, checkfirst=True))
