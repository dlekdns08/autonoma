...
// The character sits with its feet at the motion groundY. useAgentMotion
// returns y as a percent already mapped to STAGE.groundY-ish behaviour.
// Percent-based sizing keeps everything synced with the canvas scale.
const CHAR_W_PCT = (CHAR.width / STAGE.width) * 100; // 5%
const CHAR_H_PCT = (CHAR.height / STAGE.height) * 100; // 12.5%

export default function Stage({ agents, sky }: Props) {
  const skyMode = resolveSky(sky);
  const theme: MapTheme = "meadow";
  const motions = useAgentMotion({ agents, groundYPercent: 82 });
...
function AgentOnMap({ agent, motion }: { agent: AgentData; motion: MotionState }) {
  ...
  return (
    ...
    >
      {agent.speech && (
        <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-1 max-w-[200px] whitespace-nowrap">
      ...
      <div
        className="absolute top-full left-1/2 -translate-x-1/2 mt-0.5 flex flex-col items-center whitespace-nowrap"
      >
        <div
